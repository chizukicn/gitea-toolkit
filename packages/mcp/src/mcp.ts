...
server.registerTool(
  ...
  async ({ owner, repo, state, page, limit, base, sort, milestone, labels, poster, login }) => {
    const client = getClient(login);
    const pulls = await client.listPulls(owner, repo, { state, page, limit, base, sort, milestone, labels, poster });
